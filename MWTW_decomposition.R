...
data = firm_industry_balanced_panel[three_digit_one_obs_per_firm, on = c('GlobalCompanyKey', 'calendaryear', 'my_three_digit_NAICS'), three_digit_ratio := i.three_digit_ratio]
...
firms_and_industries = data[!my_three_digit_NAICS %in% c(221, 521, 522, 523, 524, 525) & !is.na(my_three_digit_NAICS) & !is.na(monopolywealth) &!is.na(totalwealth),
                            ...
                            ][firm_TW > 0][, in_sampled_industries := sum(three_digit_ratio), .(GlobalCompanyKey, calendaryear)][in_sampled_industries > 0]
